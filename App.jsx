...
/**
 * [KeyWord NewsBot v7.0 - Professional Branding Edition]
 * - 로고 최적화: 생성된 로고 디자인 톤에 맞춘 앱 아이콘 및 테마 적용
 * - PWA 완성: 홈 화면 추가 시 'Keyword Newsbot' 이름과 전용 돋보기 로고 고정
 * - 데이터 독립성: 사용자별 전용 경로 저장 (Rule 1)
 * - 티커 정밀도: 미국(USD) 소수점 1자리, 한국(원) 정수 표기
 */
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'keyword-newsbot-v7';
...
export default function App() {
  ...
  // PWA 및 앱 브랜딩 최적화
  useEffect(() => {
    // 생성된 로고 컨셉의 SVG 데이터 (청록색 계열 돋보기 + 디지털 그리드 느낌)
    const iconSvg = `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%2314b8a6' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'%3E%3Ccircle cx='11' cy='11' r='8'/%3E%3Cline x1='21' y1='21' x2='16.65' y2='16.65'/%3E%3Cpath d='M11 3a8 8 0 0 1 8 8' stroke-opacity='0.3'/%3E%3C/svg%3E`;
    
    document.title = "Keyword Newsbot";

    const metaTags = [
      { name: 'apple-mobile-web-app-capable', content: 'yes' },
      { name: 'apple-mobile-web-app-status-bar-style', content: 'black-translucent' },
      { name: 'apple-mobile-web-app-title', content: 'Keyword Newsbot' },
      ...
      const results = await Promise.all(tickers.map(async (t) => {
        ...
        try {
          ...
          if (data?.chart?.result?.[0]) {
            ...
            if (curPrice === undefined) throw new Error();
  ...
  const searchNews = async () => {
        ...
        if (data.items) {
          results.push({
            ...
            articles: data.items.slice(0, 4).map(item => {
              ...
              const dateStr = `${pDate.getFullYear()}.${String(pDate.getMonth() + 1).padStart(2, '0')}.${String(pDate.getDate()).padStart(2, '0')} ${String(pDate.getHours()).padStart(2, '0')}:${String(pDate.getMinutes()).padStart(2, '0')}`;
              const splitTitle = item.title.split(' - ');
              return { title: splitTitle[0], link: item.link, source: splitTitle[1] || "뉴스", date: dateStr };
            }),
          });
        }
      }
    ...
    } catch (err) { setStatusMsg("검색 오류 발생"); }
  ...
  const addTickerAction = async (e) => {
    ...
    try {
      ...
      if (data?.chart?.result?.[0]?.meta) {
        const meta = data.chart.result[0].meta;
        const resolvedName = meta.shortName || symbol;
    ...
    <div className="min-h-screen bg-[#F1F5F9] font-sans text-slate-900 pb-12 overflow-x-hidden">
      ...
      <section className="bg-[#0F172A] text-white border-b border-teal-500/40 overflow-hidden h-7 flex items-center relative z-50 shadow-sm">
        <div className="flex whitespace-nowrap animate-marquee">
          ...
          {marketData.length === 0 && <span className="text-[8px] text-slate-500 px-10 tracking-widest uppercase">Initializing Ticker...</span>}
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest mt-1">Intelligence Monitor v7.0</p>
            </div>
          </div>
          ...
          <div className="p-4 md:p-5 space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex items-center bg-slate-50 rounded-lg p-0.5 border border-slate-200">
                ...
                <select value={lookback} onChange={(e) => setLookback(Number(e.target.value))} className="bg-white text-slate-600 text-[9px] font-black px-2 py-1 rounded-md border border-slate-100 focus:ring-1 focus:ring-teal-500 outline-none cursor-pointer min-w-[70px]">
            ...
            <div className="flex flex-col md:flex-row gap-2">
              <form onSubmit={addKeywordAction} className="flex-1 relative group">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-teal-500 transition-colors" size={16} />
                <input type="text" value={newKeyword} onChange={(e) => setNewKeyword(e.target.value)} placeholder="키워드를 입력하세요" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-3 py-2.5 text-sm font-bold text-slate-700 focus:bg-white focus:border-teal-500 outline-none transition-all placeholder:text-slate-300 shadow-inner" />
            ...
            <div className="flex flex-wrap gap-1.5 pt-1 border-t border-slate-50">
              {keywords.map((kw) => (
                <div key={kw.id} className="bg-white border border-slate-200 pl-3 pr-1.5 py-1.5 rounded-lg text-[10px] font-bold text-slate-600 flex items-center gap-2 shadow-sm group hover:border-teal-300">{kw.name}<button onClick={() => deleteKeyword(kw.id)} className="text-slate-300 hover:text-rose-500 transition-colors"><X size={10} /></button></div>
              ))}
            </div>
          </div>
        </section>
        ...
        {report && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-3 duration-500 pb-10">
            {report.map((res, idx) => {
              ...
              return (
                <div key={idx} className={`bg-white rounded-2xl border-t-4 ${theme.border} p-3.5 shadow-md transition-all hover:shadow-lg`}>
                  ...
                  <div className="space-y-1.5">
                    {res.articles.map((art, aIdx) => (
                      <a key={aIdx} href={art.link} target="_blank" rel="noreferrer" className="block p-2.5 rounded-xl bg-slate-50 border border-transparent hover:bg-white hover:border-teal-100 hover:shadow-sm transition-all group">
                        ...
                        <div className="flex items-center justify-between text-[8px]">
                          <div className="flex items-center gap-1.5"><div className={`w-1 h-1 rounded-full ${theme.dot} opacity-40`}></div><span className="font-black text-slate-400 uppercase">{art.source}</span><span className="font-bold text-slate-300">{art.date}</span></div>
      ...
      {/* 상태 알림 */}
      {statusMsg && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-2.5 rounded-full shadow-2xl z-[100] flex items-center gap-3 animate-in slide-in-from-bottom-6 border border-teal-500/20">
          ...
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-4">
              <form onSubmit={addTickerAction} className="flex gap-2">
                <input type="text" value={newTickerSymbol} onChange={e=>setNewTickerSymbol(e.target.value)} placeholder="심볼(예: NVDA, AAPL)" className="flex-1 bg-slate-50 border rounded-xl px-4 py-3 text-sm font-bold uppercase outline-none focus:border-teal-500 shadow-inner" disabled={isAddingTicker} />
                <button type="submit" className="bg-teal-600 text-white px-5 rounded-xl font-black disabled:opacity-50" disabled={isAddingTicker || !newTickerSymbol.trim()}>{isAddingTicker ? <Loader2 className="animate-spin" size={18} /> : "추가"}</button>
              </form>
              <div className="max-h-60 overflow-y-auto space-y-2 pr-1 border-t pt-4">
                {tickers.map(tk => (
                  <div key={tk.id} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border">
                    <div className="flex flex-col"><span className="text-xs font-black">{tk.name}</span><span className="text-[10px] text-slate-400 font-mono tracking-tight">{tk.symbol}</span></div>
                    <button onClick={() => deleteTicker(tk.id)} className="text-slate-300 hover:text-rose-500 transition-all"><Trash2 size={16} /></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-xs p-6 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-6">
              <div className="space-y-2 text-center"><label className="text-[8px] font-black text-slate-400 uppercase block">Report Email Address</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold text-slate-700 outline-none focus:border-teal-500 shadow-inner" /></div>
              <button onClick={() => setShowSettings(false)} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-sm shadow-xl active:scale-95 transition-all">설정 완료</button>
            </div>
          </div>
        </div>
      )}
      ...
      <footer className="py-12 text-center opacity-20 select-none">
        <p className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-400">© 2024 Keyword Intelligence • Stable v7.0</p>
      </footer>
    </div>
  );
}
